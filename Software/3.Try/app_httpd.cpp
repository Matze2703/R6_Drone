...
void send_wav_header(httpd_req_t *req, uint32_t sample_rate, uint16_t bits_per_sample, uint16_t channels) {
    ...
    *(uint32_t*)(wav_header + 4) = data_size + 36;
    memcpy(wav_header + 8, "WAVEfmt ", 8);
    *(uint32_t*)(wav_header + 16) = 16;
    *(uint16_t*)(wav_header + 20) = 1;
    ...
    *(uint16_t*)(wav_header + 32) = channels * bits_per_sample / 8;
...
esp_err_t wav_audio_handler(httpd_req_t *req) {
    send_wav_header(req, 16000, 16, 1);
    const int buffer_size = 1024;
    int16_t buffer[buffer_size];
    size_t bytes_read = 0;

    while (true) {
        esp_err_t ret = i2s_read(I2S_NUM_0, (void*)buffer, buffer_size * sizeof(int16_t), &bytes_read, portMAX_DELAY);
...
// Ab dem Punkt weiß ich nicht mehr, welche Funktion was macht, das kommt größtenteils schon aus dem vorgegebenen Beispielcode
